...
# IMPORTANT: CrewAI applies `str.format(**inputs)` to each task description
# at kickoff time. Any literal `{` / `}` in the description (e.g. JSON
# examples) MUST be doubled to `{{` / `}}` to escape them -- otherwise Python
# treats e.g. `{"batch_id": ...}` as a format spec and raises
# `KeyError: '"batch_id"'`. The real placeholders we DO want substituted
# (currently just `{run_id}`) remain single-braced.
